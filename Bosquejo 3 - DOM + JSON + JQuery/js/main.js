...
//SE VAN IMPRIMIENDO LAS TAREAS A MEDIDA QUE VAN PASANDO SUS TIEMPOS.
function impresionTemporizada(){
    $("#reset").toggle("slow");
    //SE ALTERNA EL COLOR DEL BACKGROUND CADA 3 SEGUNDOS.
    let animarBackground = setInterval(()=>{
        console.log(contadorBackground, contadorTareas);
        ...
        if (contadorTareas == listaTareas.length){
            console.log("Se salio del intervalo de background");
            $("body").css({'backgroundColor': "#4267B2"});
    ...
    for (const tarea of listaTareas){
      ...
      //playSound();
      //$("#salidaListaTareas").children(0).append(`<h3 style="display: none;" id="tarea${tarea.id}">
      //                                Fin de tarea ${tarea.id}: ${tarea.nombreTarea}
      //                                </h3> `);
      //console.log("Hola!");
      //$(`#tarea${tarea.id}`).delay(auxiliarTimer).toggle("slow");
      cuentaRegresiva(auxiliarTimer, tarea.id);
      console.log(auxiliarTimer);
      if (tarea.id != 1){
...
function cuentaRegresiva (counter, tareaId){
    ...
    let interval = setInterval (()=>{ //SE REPITE LA FUNCION ANONIMA CADA 1 SEGUNDO.
        //console.log(counter);
        $(`#clock${tareaId}`).html(`Tarea ${tareaId} segundos restantes: ${(counter -= 1000) / 1000}`); //SE SOBREESCRIBE EL HTML CADA VEZ QUE SE EJECUTA LA FUNCION PARA INDICAR LOS SEGUNDOS RESTANTES.
        if (counter ==0){ //CUANDO EL COUNTER LLEGA A CERO SE LANZA LA NOTIFICACION, SE FRENA El INTERVAL Y SE MUESTRA LA TAREA SIGUIENTE.
            ...
            $(`#clock${tareaId}`).hide();
            $(`#clock${tareaId}`).html(`Tarea ${tareaId} FINALIZADA`);
            $(`#clock${tareaId}`).toggle("slow");
